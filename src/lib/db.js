...
const seedProjects = [
  {
    ...
    description: "A playground to play Conway's Game Of Life. Developed using React and TypeScript.",
...
function normalizeKeywordsInput(value) {
  ...
  return list.map((keyword) => keyword?.toString().trim()).filter((keyword) => Boolean(keyword));
}
...
function mapRow(row) {
  return {
    ...row,
    keywords: Array.isArray(row.keywords) ? row.keywords : normalizeKeywordsInput(row.keywords),
  };
}
...
export async function updateProject(id, updates = {}) {
  ...
  const nextKeywords = updates.keywords !== undefined ? normalizeKeywordsInput(updates.keywords) : current.keywords;
